...
// 1) Prozentwerte berechnen und Beschriftungen für Donut-Diagramm definieren
const totalOutcomes = outcomeCounts.home + outcomeCounts.draw + outcomeCounts.away;
const homePct = ((outcomeCounts.home / totalOutcomes) * 100).toFixed(1);
const drawPct = ((outcomeCounts.draw / totalOutcomes) * 100).toFixed(1);
const awayPct = ((outcomeCounts.away / totalOutcomes) * 100).toFixed(1);

// Labels (für Legende) und Texte (nur Kürzel) bereitstellen
const legendLabels = [
  `${homeTeam} gewinnt ${homePct}%`,
  `UNENTSCHIEDEN ${drawPct}%`,
  `${awayTeam} gewinnt ${awayPct}%`
];
const pieTexts = [homeTeam, 'UN', awayTeam];
  return (
      ...
      <div ref={chartAreaRef}>
        {resultMatrix.length > 0 && (
          <div ref={plotRef} className="relative p-4 bg-[#36393f] rounded">
            ...
            <div className="mt-1 bg-[#36393f] p-4 rounded">
  <h3 className="text-left text-white font-bold text-lg mb-4">Sieg / Remis / Niederlage</h3>
  <Plot
  data={[{
    values: [outcomeCounts.home, outcomeCounts.draw, outcomeCounts.away],
    labels: legendLabels,          // Legende: Kürzel + Prozent
    text: pieTexts,               // Text im Donut: nur Kürzel
    textinfo: 'text',             // nur 'text' anzeigen
    hoverinfo: 'label',           // Legend-Labels als Hover-Info
    type: 'pie',
    hole: 0.3,
    marker: {
      colors: [
...
<div className="mt-1 bg-[#36393f] p-4 rounded">
  <h3 className="text-left text-white font-bold text-lg mb-4">Torverteilung</h3>
  <Plot 
    data={[
      {
  ...
  />
            ...
            <div className="absolute flex flex-col items-center z-50" style={{ top: '1300px', left: '-45px', transform: 'rotate(-90deg)' }}>
...
<div className="absolute flex items-center z-50" style={{ top: '1580px', left: '300px' }}>
  <button className="background-color: rgb(55 65 81); text-white h-[40px] w-[120px] flex items-center justify-center text-center rounded cursor-default">
...
<Plot
            data={[
              /* Cast to any to satisfy TS typing */
              ({
                z: resultMatrix,
                text: resultMatrix.map(row => row.map(val => `${val.toFixed(1)}%`)) as any,
                texttemplate: "%{text}",
...
<div className="text-center text-xl font-bold space-y-04 bg-[#36393f] rounded p-4">
