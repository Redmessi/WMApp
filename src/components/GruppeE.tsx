// src/components/GruppeE.tsx
import { useContext, useEffect } from 'react';
import { ResultsContext, MatchResult } from "../contexts/ResultsContext";

const teamsData = [
  { code: 'ESP', land: 'Spanien' },
  { code: 'TUR', land: 'Türkei' },
  { code: 'GEO', land: 'Georgien' },
  { code: 'BUL', land: 'Bulgarien' }
];

const initialSpiele: MatchResult[] = [
  { spieltag: 1, datum: '04.09.2025', heim: 'GEO', auswärts: 'TUR', homeGoals: '', awayGoals: '' },
  { spieltag: 1, datum: '04.09.2025', heim: 'BUL', auswärts: 'ESP', homeGoals: '', awayGoals: '' },
  { spieltag: 2, datum: '07.09.2025', heim: 'GEO', auswärts: 'BUL', homeGoals: '', awayGoals: '' },
  { spieltag: 2, datum: '07.09.2025', heim: 'TUR', auswärts: 'ESP', homeGoals: '', awayGoals: '' },
  { spieltag: 3, datum: '11.10.2025', heim: 'BUL', auswärts: 'TUR', homeGoals: '', awayGoals: '' },
  { spieltag: 3, datum: '11.10.2025', heim: 'ESP', auswärts: 'GEO', homeGoals: '', awayGoals: '' },
  { spieltag: 4, datum: '14.10.2025', heim: 'TUR', auswärts: 'GEO', homeGoals: '', awayGoals: '' },
  { spieltag: 4, datum: '14.10.2025', heim: 'ESP', auswärts: 'BUL', homeGoals: '', awayGoals: '' },
  { spieltag: 5, datum: '15.11.2025', heim: 'GEO', auswärts: 'ESP', homeGoals: '', awayGoals: '' },
  { spieltag: 5, datum: '15.11.2025', heim: 'TUR', auswärts: 'BUL', homeGoals: '', awayGoals: '' },
  { spieltag: 6, datum: '18.11.2025', heim: 'BUL', auswärts: 'GEO', homeGoals: '', awayGoals: '' },
  { spieltag: 6, datum: '18.11.2025', heim: 'ESP', auswärts: 'TUR', homeGoals: '', awayGoals: '' },
];

const flagge = (code: string) => `/flags/${code.toLowerCase()}.svg`;
const landName = (code: string) => teamsData.find(t => t.code === code)?.land || code;

export default function GruppeE() {
  const { results, setResults } = useContext(ResultsContext);

  useEffect(() => {
    if (!results["E"]) {
      setResults(r => ({ ...r, E: initialSpiele }));
    }
  }, [results, setResults]);

  const spiele = results["E"] ?? initialSpiele;

  const updateGoals = (idx: number, field: 'homeGoals' | 'awayGoals', value: string) => {
    setResults(r => {
      const group = [...(r["E"] ?? initialSpiele)];
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      <h2 className="text-2xl font-bold">🌍 WM-Quali 2026 – Gruppe E</h2>
